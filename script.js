document.addEventListener("DOMContentLoaded", () => {
    ...
    function handleTilt(e) {
        ...
        // Calculate rotation amounts (Subtle 3D, max 4 degrees)
        const maxTilt = 4;
        const rotateX = ((y - centerY) / centerY) * -maxTilt;
        const rotateY = ((x - centerX) / centerX) * maxTilt;
        
        // Custom scale if requested
        const scale = el.getAttribute('data-tilt-scale') || '1.02';
    ...
    // 2. Logo interactive bounce pulse
    const logo = document.querySelector('.logo');
    if(logo) {
        logo.addEventListener('mouseover', () => {
             logo.style.transform = 'translateZ(15px) scale(1.05) rotate(2deg)';
        });
